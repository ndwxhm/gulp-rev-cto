...
plugin.manifest = (pth, opts) => {
    ...
    return through.obj((file, enc, cb) => {
        ...
        const fileRootPath = relPath(file.cwd, file.base);
        const resourceType = path.basename(fileRootPath)
        let { resourceUrl } = opts
        const originalFile = path.join(opts.rootPath?resourceType:"", path.dirname(revisionedFile), path.basename(file.revOrigPath)).replace(/\\/g, '/');
        transformFilename(file);
        manifest[originalFile] = `${resourceUrl}${originalFile}?${opts.revMark}=${file.revHash}`;
...
module.exports.revCollector = require('./rev-collector');
